...
function route(target) {
    ...
    if (target === 'vehicle-vote') renderVehicleVote();
}
...
window.openHouseManage = function (houseId) {
    ...
    const squadBadge = document.getElementById('house-edit-squad-badge');
    if (squadBadge) {
        squadBadge.innerText = (house.squad && house.squad !== 0) ? `Squad ${house.squad}` : 'Unassigned';
    }
...
// VEHICLE & VOTE CHECK
// ---------------------------
function renderVehicleVote() {
    const container = document.getElementById('vehicle-vote');
    
    // Squad selector row
    container.innerHTML = `
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom: 20px;">
            <h1 style="margin:0;">Vehicle & Vote Check</h1>
        </div>
        <div class="squad-tabs-row" id="vv-squad-tabs">
            ${SQUADS.map(num => `
                <div class="card" style="text-align:center; cursor:pointer;" onclick="renderVehicleVoteSquad(${num})">
                    <h2>Squad ${num}</h2>
                    <p>${voters.filter(v => v.squad === num).length} Voters</p>
                </div>
            `).join('')}
            <div class="card" style="text-align:center; cursor:pointer; border:1px dashed var(--primary); background:#fdf9f0;" onclick="renderVehicleVoteSquad(0)">
                <h2 style="color:#f97316;">No Mapping</h2>
                <p><strong>${voters.filter(v => !v.houseId).length}</strong> Voters</p>
            </div>
        </div>
        <div id="vv-squad-details" style="display:none; margin-top:20px;"></div>
    `;

    if (currentVVSquad !== null) {
        window.renderVehicleVoteSquad(currentVVSquad);
    }
}

window.renderVehicleVoteSquad = function(squadNum) {
    currentVVSquad = squadNum;
    const details = document.getElementById('vv-squad-details');
    details.style.display = 'block';
    
    let squadHouses = [];
    let unassigned = [];
    if (squadNum === 0) {
        unassigned = voters.filter(v => !v.houseId).sort((a, b) => a.serialNo - b.serialNo);
    } else {
        squadHouses = houses.filter(h => h.squad === squadNum).sort((a, b) => (a.order || 0) - (b.order || 0));
    }

    if (squadNum === 0) {
        details.innerHTML = `
            <h2 style="margin-bottom:15px; color:#f97316;">Unassigned Voters (No Mapping)</h2>
            <div class="card" style="overflow-x:auto;">
                <table class="data-table">
                    <thead>
                        <tr><th style="padding:10px;">Sl No</th><th>Name</th><th>Original House</th><th style="text-align:center;">Will Vote</th><th style="text-align:center;">Vehicle Needed</th></tr>
                    </thead>
                    <tbody>
                        ${unassigned.map(v => `
                            <tr>
                                ...
                                <td><small>${v.houseName}</small></td>
                                <td style="text-align:center;">
                                    <input type="checkbox" id="vv-will-${v.serialNo}" ${v.willVote !== false ? 'checked' : ''} onchange="updateVV(${v.serialNo}, 'willVote', this.checked)" style="width:22px; height:22px; cursor:pointer;">
                                </td>
                                <td style="text-align:center;">
                                    <input type="checkbox" id="vv-veh-${v.serialNo}" ${v.vehicleNeeded ? 'checked' : ''} onchange="updateVV(${v.serialNo}, 'vehicleNeeded', this.checked)" style="width:22px; height:22px; cursor:pointer;">
                                </td>
                            </tr>
                        `).join('')}
                    </tbody>
        ...
        details.innerHTML = `
            <h2 style="margin-bottom:15px;">Squad ${squadNum} Checklist</h2>
            <div style="display:flex; flex-direction:column; gap:15px;">
                ${squadHouses.map(h => {
                    const members = voters.filter(v => v.houseId === h.id).sort((a, b) => a.serialNo - b.serialNo);
                    if (members.length === 0) return '';
                    return `
                        <div class="card" style="padding:0; overflow:hidden;">
                            <h4 style="background:var(--bg-color); padding:10px 15px; margin:0; border-bottom:1px solid var(--border);">${h.name} <small class="text-muted">(${members.length} members)</small></h4>
                            <div style="overflow-x:auto; padding:10px;">
                                <table class="data-table" style="margin:0;">
                                    <tr>
                                        <th style="width:10%;">Sl No</th>
                                        <th style="width:40%;">Voter Name</th>
                                        <th style="width:25%; text-align:center;">Will Vote</th>
                                        <th style="width:25%; text-align:center;">Vehicle Needed</th>
                                    </tr>
                                    ${members.map(m => `
                                        <tr>
                                            <td><strong>#${m.serialNo}</strong></td>
                                            <td>${m.name} ${m.hasPolled ? '<span class="badge polled" style="font-size:0.7em;">Polled</span>' : ''}</td>
                                            <td style="text-align:center;">
                                                <input type="checkbox" id="vv-will-${m.serialNo}" ${m.willVote !== false ? 'checked' : ''} onchange="updateVV(${m.serialNo}, 'willVote', this.checked)" style="width:22px; height:22px; cursor:pointer;">
                                            </td>
                                            <td style="text-align:center;">
                                                <input type="checkbox" id="vv-veh-${m.serialNo}" ${m.vehicleNeeded ? 'checked' : ''} onchange="updateVV(${m.serialNo}, 'vehicleNeeded', this.checked)" style="width:22px; height:22px; cursor:pointer;">
                                            </td>
                                        </tr>
                                    `).join('')}
                                </table>
                ...
                ${squadHouses.length === 0 ? '<p class="text-muted">No houses assigned to this squad.</p>' : ''}
            </div>
        `;
    }
}

window.updateVV = function(sno, field, value) {
    const voter = voters.find(v => v.serialNo === sno);
    if(voter) {
        voter[field] = value;
        rtdb.ref(`voters/${sno}/${field}`).set(value).catch(e => {
            alert("Error syncing to cloud: " + e.message);
        });
    }
}
...
window.toggleMissingList = function (party, btn) {
    ...
    if (container.style.display === 'none' || container.getAttribute('data-current-party') !== party) {
        ...
        let filtered;
        if (party === 'Neutral') {
            filtered = voters.filter(v => !v.hasPolled && v.willVote !== false && (!v.politics || v.politics === 'Unknown' || v.politics.includes('Neutral')));
        } else {
            filtered = voters.filter(v => !v.hasPolled && v.willVote !== false && v.politics && v.politics.includes(party));
        }
...
function renderPolling() {
    ...
    const ldfPolled = voters.filter(v => v.hasPolled && v.willVote !== false && v.politics && v.politics.includes('LDF')).length;
    const udfPolled = voters.filter(v => v.hasPolled && v.willVote !== false && v.politics && v.politics.includes('UDF')).length;
    const ndaPolled = voters.filter(v => v.hasPolled && v.willVote !== false && v.politics && v.politics.includes('NDA')).length;
    const neutralPolled = voters.filter(v => v.hasPolled && v.willVote !== false && (!v.politics || v.politics === 'Unknown' || v.politics.includes('Neutral'))).length;

    const unpolledVoters = voters.filter(v => !v.hasPolled && v.willVote !== false).sort((a, b) => a.serialNo - b.serialNo);
